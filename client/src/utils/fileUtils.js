 ...
 * @param {string} props.iconKey - The icon key from techIcons
 * @param {number} props.size - Icon size (default: 16)
 * @param {string} props.className - CSS class name (default: "file-icon")
 * @param {Object} props.style - Inline styles
 * @returns {JSX.Element|null} - The SVG icon component or null if not found
 */
export const SVGIcon = ({ iconKey, size = 16, className = "file-icon", style = {} }) => {
 ...
 * @param {boolean} isDirectory - Whether the file is a directory
 * @returns {JSX.Element} - The file icon component
 */
...
export const isTextFile = (fileName) => {
  const textExtensions = [
    ...
    'md', 'markdown', 'mdx', 'txt',
    'py', 'java', 'c', 'cpp', 'h', 'cs', 'php', 'rb', 'rs', 'go', 'kt', 'swift', 'dart',
    'sh', 'bash', 'zsh', 'fish', 'ps1',
    'sql', 'vue', 'svelte',
    'env', 'ini', 'conf', 'config', 'properties',
  ...
  // Check special files
  if (name === 'dockerfile' || name === '.gitignore' || name === '.gitattributes') {
    return true;
  }
 ...
 * @param {string} fileName - The file name
 * @returns {boolean} - True if file is a binary file
 */
export const isBinaryFile = (fileName) => {
  ...
  return binaryExtensions.includes(getFileExtension(fileName));
};
...
export const getFileType = (fileName) => {
  ...
  if (['env', 'ini', 'conf', 'config', 'properties'].includes(extension)) {
    return 'config';
  }
